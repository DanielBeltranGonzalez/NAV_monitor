...
describe('GET /api/banks', () => {
  it('devuelve lista de bancos', async () => {
    const banks = [{ id: 1, name: 'Bankinter' }]
...
describe('POST /api/banks', () => {
  it('crea banco con nombre válido → 201', async () => {
    const bank = { id: 1, name: 'Bankinter' }
    ;(prisma.bank.create as jest.Mock).mockResolvedValue(bank)
...
describe('PATCH /api/banks/[id]', () => {
  it('actualiza nombre → 200', async () => {
    const bank = { id: 1, name: 'Sabadell' }
    ;(prisma.bank.update as jest.Mock).mockResolvedValue(bank)
